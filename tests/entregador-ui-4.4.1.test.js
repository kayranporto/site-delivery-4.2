...
const js = fs.readFileSync(path.join(raiz, "js/entregador-ui-4.4.1.js"), "utf8");
const css = fs.readFileSync(path.join(raiz, "css/entregador-4.4.1.css"), "utf8");

test("painel do entregador carrega a camada operacional 4.4.1", () => {
  assert.match(html, /css\/entregador-4\.4\.1\.css\?v=4\.4\.1/);
  assert.match(html, /js\/entregador-ui-4\.4\.1\.js\?v=4\.4\.1/);
});
...
test("ações críticas continuam pertencendo ao fluxo base", () => {
  assert.match(html, /js\/entregador\.js\?v=4\.2\.0/);
  assert.match(html, /js\/entregador-logistica-4\.4\.js\?v=4\.4\.5/);
});
